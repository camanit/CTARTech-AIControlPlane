...
export default function ReportsPage() {
  ...
  const [reportType, setReportType] = useState<'AUDIT_LOGS' | 'ISO_PDP_ATTESTATION'>('AUDIT_LOGS');
  ...
  return (
    <div className="flex min-h-screen bg-slate-950 text-slate-100 font-sans">
      ...
      <main className="flex-1 p-8 max-w-5xl mx-auto overflow-y-auto w-full">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-xl font-bold uppercase tracking-wide text-black">
                CTARTech-AIControlPlane &bull; Official Compliance &amp; Governance Attestation
              </h1>
              <p className="text-xs text-gray-600">Enterprise AI Agent Runtime Governance &bull; ISO/IEC 27001, ISO 22301, ISO 9001 &amp; UU PDP No. 27/2022</p>
            </div>
            <div className="text-right text-xs font-mono">
              <div>Organisasi: {stats?.org_name || 'PT CTARTech Global Enterprise'}</div>
        ...
        <div className="no-print mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 text-xs font-mono text-cyan-400 uppercase tracking-wider mb-2">
              ...
              <span>Compliance &amp; Legal Governance Reports</span>
            </div>
            <h1 className="text-2xl font-extrabold text-white">Laporan Kepatuhan &amp; Cetak Peristiwa AI</h1>
            <p className="text-xs text-slate-400 mt-1">
              Rekonstruksi keputusan operasional dan sertifikat kepatuhan resmi untuk audit SOC2 / NIST / ISO / UU PDP.
            </p>
          </div>
        ...
        <div className="no-print flex items-center gap-3 mb-6">
          ...
          >
            ...
            <span>1. Log Audit Peristiwa Runtime</span>
          </button>
          <button
            onClick={() => setReportType('ISO_PDP_ATTESTATION')}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${
              reportType === 'ISO_PDP_ATTESTATION'
                ? 'bg-cyan-500 text-slate-950 shadow-md shadow-cyan-500/20'
                : 'bg-slate-900 text-slate-400 border border-slate-800 hover:text-white'
            }`}
          >
            <Award className="w-3.5 h-3.5" />
            <span>2. Surat Pernyataan Kepatuhan ISO &amp; UU PDP</span>
          </button>
        </div>

        {/* REPORT TYPE 1: RUNTIME AUDIT TRAIL */}
        {reportType === 'AUDIT_LOGS' && (
          <div>
        ...
        {/* REPORT TYPE 2: EXECUTIVE ISO & UU PDP COMPLIANCE ATTESTATION */}
        {reportType === 'ISO_PDP_ATTESTATION' && (
          <div className="p-8 rounded-2xl bg-slate-900 border border-slate-800 glass-panel space-y-6">
            <div className="border-b border-slate-800 pb-4">
              <div className="text-xs font-mono text-cyan-400 uppercase font-bold">Official Compliance Attestation</div>
              <h2 className="text-xl font-extrabold text-white mt-1">
                Pernyataan Kepatuhan Standar Keamanan &amp; Privasi Data Agen AI
              </h2>
              <p className="text-xs text-slate-400 mt-1">
                Diterbitkan secara sah untuk kualifikasi audit perbankan (BFSI), enterprise, dan institusi pemerintahan.
              </p>
            </div>
            ...
            <div className="space-y-4 text-xs leading-relaxed text-slate-300">
              <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800">
                <div className="font-bold text-cyan-300 mb-1 flex items-center gap-2">
                  ...
                  <span>1. ISO/IEC 27001:2022 (ISMS &amp; Cryptographic Evidence)</span>
                </div>
                <p className="text-slate-400">
                  Enkripsi end-to-end (AES-256 GCM &amp; TLS 1.3), isolasi data multi-tenant, serta kontrol akses berbasis peran (RBAC/ABAC) terintegrasi pada gateway runtime Rust Axum. Kunci privat lisensi Ed25519 terlindungi secara mutlak (*Zero-Leakage Guarantee*).
                </p>
              </div>
              ...
              <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800">
                <div className="font-bold text-cyan-300 mb-1 flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-emerald-400" />
                  <span>2. ISO 22301:2019 (Business Continuity Management &amp; Stress-Test Resilience)</span>
                </div>
                <p className="text-slate-400">
                  Teruji dengan Stress-Test Simulator berkapasitas 4.850+ TPS, latensi p99 sub-milidetik (0.78ms), dan mekanisme Time-Based SLA Escalation otomatis untuk mencegah terjadinya disrupsi operasional bisnis.
                </p>
              </div>
              ...
              <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800">
                <div className="font-bold text-cyan-300 mb-1 flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-emerald-400" />
                  <span>3. ISO 9001:2015 (Quality Management System &amp; Root Cause Analytics)</span>
                </div>
                <p className="text-slate-400">
                  Mandatory Root Cause Analytics (RCA Gate) dengan analisis 5-Whys otomatis dan sinkronisasi SOP kebijakan berkelanjutan ke OPA Dynamic Policy Engine.
                </p>
              </div>
              ...
              <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800">
                <div className="font-bold text-cyan-300 mb-1 flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-emerald-400" />
                  <span>4. UU PDP No. 27/2022 &amp; EU GDPR (Personal Data Protection)</span>
                </div>
                <p className="text-slate-400">
                  Dynamic PII Data Masking secara otomatis menyensor NIK, nomor rekening, kontak pribadi, dan identitas finansial sebelum dicatat ke log audit. Retensi log 90 hari dengan penghapusan permanen (*cryptographic shredding*).
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="hidden print-only mt-12 pt-8 border-t border-gray-400 flex justify-between text-xs text-gray-800">
          <div>
            ...
            <p>Standar Kepatuhan: ISO/IEC 27001 &bull; ISO 22301 &bull; ISO 9001 &bull; UU PDP No. 27/2022</p>
          </div>
          <div className="text-right">
            <p>Disetujui oleh CISO / Compliance Lead:</p>
            <div className="mt-12 font-bold underline">( CISO_Compliance_Lead )</div>
            <p className="text-[10px] text-gray-500">Security Operations &amp; Audit Department</p>
          </div>
        </div>
      </main>
    </div>
